...
/**
 * Turning what somebody types into tags, and tags back into something somebody
 * can read.
 *
 * A port of the desktop client's `mention-utils.ts`. Reading tags always worked
 * -- `MessageContent` draws an incoming `<@id>` as a name -- and this is the
 * writing half, which is what the README listed as the reason composing a
 * mention was not offered at all. The longest-match rules below are the whole of
 * that reason: a half-working version silently sends the wrong person's name as
 * ordinary text, which is worse than no feature, so it was ported whole rather
 * than approximated.
 *
 * The composer is a plain `TextInput` and stays one, for exactly the reason the
 * desktop composer stays a textarea: a rich editor would hold the ids
 * invisibly and there would be nothing to convert, but it would also mean
 * re-implementing selection, undo and paste -- and on this platform all three
 * are the system's job and are better than anything written here would be.
 *
 * So the box holds names, exactly as they are read, and the conversion to
 * `<@id>` happens once on the way out. Everything in this file is pure.
 */
...
function candidates(
  ...
  for (const u of users) {
    ...
    // Only when it is not the same string, or "@bob" would offer two identical
    // rows for one person.
  ...
  return out.sort(
    (a, b) =>
      ...
      // Ties among people who genuinely share a name resolve the same way every
      // time rather than by whatever order the member list arrived in.
 ...
 * when two people answer to the same string -- then the one actually tapped
...
export function matchUsers(
  ...
  return scored
    .sort(
      (a, b) =>
        b.score - a.score || mentionName(a.user).localeCompare(mentionName(b.user)),
    )
    .slice(0, limit)
    .map((s) => s.user);
}
